...
trait ShopTradeServicePart5
{
    private function sellLegacy(int $campaignId, array $payload, array $authContext, ?string $idempotencyKey = null): array
    {
        ...
        if (!$ownerCode || !$shopId || !$selections) {
            ...
            $this->logTransaction($campaignId, 'SELL', $idempotencyKey, $authContext, $ownerCode ?: null, $shopId ?: null, $payload, $error, 'FAILED', 'invalid_payload', 0);
            return $error;
        }

        $cached = $this->resolveIdempotent($campaignId, 'SELL', $idempotencyKey, $ownerCode);
        if ($cached !== null) {
        ...
        if (!$shop) {
            ...
            $this->logTransaction($campaignId, 'SELL', $idempotencyKey, $authContext, $ownerCode, $shopId, $payload, $error, 'FAILED', 'not_found', 0);
            return $error;
        }
        if ((int) ($shop['is_active'] ?? 0) !== 1) {
            $error = $this->error('shop_inactive', 409);
            $this->logTransaction($campaignId, 'SELL', $idempotencyKey, $authContext, $ownerCode, $shopId, $payload, $error, 'FAILED', 'shop_inactive', 0);
            return $error;
        }
        ...
        $shopStockRows = $this->containerTemplateItemModel
            ->where('campaign_id', $campaignId)
            ->where('container_id', $shopContainerId)
            ->findAll();
        $shopStockByTemplateId = [];
        foreach ($shopStockRows as $row) {
            $shopStockByTemplateId[(int) ($row['template_id'] ?? 0)] = $row;
        }
        ...
        foreach ($selections as $selection) {
            ...
            if (!$templateId) {
                $error = $this->error('invalid_payload', 400);
                $this->logTransaction($campaignId, 'SELL', $idempotencyKey, $authContext, $ownerCode, $shopId, $payload, $error, 'FAILED', 'invalid_payload', 0);
                return $error;
            }
            if ($quantity <= 0) {
                $error = $this->error('invalid_quantity', 400);
                $this->logTransaction($campaignId, 'SELL', $idempotencyKey, $authContext, $ownerCode, $shopId, $payload, $error, 'FAILED', 'invalid_quantity', 0);
                return $error;
            }

            $source = $this->containerTemplateItemModel
                ->where('campaign_id', $campaignId)
                ->where('container_id', $playerContainerId)
            ...
            if ($source['quantity'] === null || (int) $source['quantity'] < $quantity) {
                ...
                $this->logTransaction($campaignId, 'SELL', $idempotencyKey, $authContext, $ownerCode, $shopId, $payload, $error, 'FAILED', 'insufficient_stock', 0);
                return $error;
            }
            ...
            $pricing = $this->pricingService->calculateForTrade(
                ...
                $profile,
                [
                    'QUANTITY' => $shopStockByTemplateId[$templateId]['quantity'] ?? 0,
            ...
            if (empty($pricing['exchangeRateConfigured']) && !$canForcePrices) {
                ...
                $this->logTransaction($campaignId, 'SELL', $idempotencyKey, $authContext, $ownerCode, $shopId, $payload, $error, 'FAILED', 'missing_exchange_rate', 0);
                return $error;
            }
        ...
        foreach ($items as $item) {
            ...
            if (!$updated || $this->db->affectedRows() !== 1) {
                ...
                $error = $this->error('insufficient_stock', 409);
                $this->logTransaction($campaignId, 'SELL', $idempotencyKey, $authContext, $ownerCode, $shopId, $payload, $error, 'FAILED', 'insufficient_stock', $total);
                return $error;
            }
            ...
            if (!$added) {
                ...
                $this->logTransaction($campaignId, 'SELL', $idempotencyKey, $authContext, $ownerCode, $shopId, $payload, $error, 'FAILED', 'transaction_failed', $total);
                return $error;
            }
        }

        $nextBalance = (int) $wallet['balance'] + $total;
        $walletCredited = $this->walletService->credit($wallet, $total);
        if (!$walletCredited) {
            $this->db->transRollback();
            $error = $this->error('transaction_failed', 500);
            $this->logTransaction($campaignId, 'SELL', $idempotencyKey, $authContext, $ownerCode, $shopId, $payload, $error, 'FAILED', 'transaction_failed', $total);
        ...
        try {
            $this->logTransaction($campaignId, 'SELL', $idempotencyKey, $authContext, $ownerCode, $shopId, $payload, $response, 'executed', null, $total, [
                'shop' => $shop,
                'items' => $items,
