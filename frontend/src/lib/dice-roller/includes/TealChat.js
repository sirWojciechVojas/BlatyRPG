...
export class TealChat {
  constructor(place, onMessageCallback) {
    this.place = place;
    this.text = Teal.element(
      "div",
      {
  ...
  insert_text(m) {
    var scroll =
      this.place.scrollHeight - this.place.scrollTop == this.place.offsetHeight;
  ...
  confirm_message(uuid, newtext, keep_uuid) {
    ...
    for (var i = 0, l = list.length; i < l; ++i) {
      ...
      if (newtext) {
        var scroll =
          this.place.scrollHeight - this.place.scrollTop ==
          this.place.offsetHeight;
        var tt = Teal.get_elements_by_class("teal-chat-text", list[i])[0];
        Teal.empty(tt);
        Teal.inner(newtext, tt);
        // Wywołanie callbacka, aby przekazać wiadomość do komponentu Vue
        if (this.onMessageCallback) {
          const messageUser = this.last_user || this.own_user;
  ...
  add_info(text, time, color) {
    ...
    var t = new Date();
    if (time) t.setTime(time);
    t = t.toTimeString().substring(0, 5);
    m.innerHTML =
      '<span class="teal-chat-time">' +
      t +
      "</span> " +
      '<span class="teal-chat-info" ' +
      (color != undefined ? ' style="color: ' + color + '"' : "") +
      ">" +
      text.replace(/\n/g, "<br/>") +
      "</span>";
    this.insert_text(m);
  ...
  add_message(user, text, time) {
    const messageElement = this.form_message(user, text, time);
    this.insert_text(messageElement);
    this.last_user = user;
  ...
  form_message(user, text, time) {
    var m = Teal.element("p", {
      class: "teal-chat-row" + (user == this.own_user ? "-own" : ""),
    });
    var t = new Date();
    if (time) t.setTime(time);
    t = t.toTimeString().substring(0, 5);
    m.innerHTML =
      '<span class="teal-chat-time">' +
      t +
      '</span> <span class="teal-chat-user">' +
      user +
      "</span><span>: </span>";
