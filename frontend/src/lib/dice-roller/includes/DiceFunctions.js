...
export class DiceFunctions {
  ...
  template(dicemeshList, args) {
    ...
    for (let i = 0, len = dicemeshList.length; i < len; ++i) {
      ...
      if (action == "force") {
        let forcedvalue =
          diceobj.values[Math.floor(Math.random() * diceobj.values.length)];
  ...
  templateHelp() {
    ...
    output["Description"] =
      "Template functions showing exmples of deleting, ignoring, modifying, and forcing dice results";
    output["Examples"] = {
      "{t}": "Does nothing.",
  ...
  rethrowBasicHelp() {
    ...
    output["Name"] = "Reroll";
    output["Usage"] =
      "{r,[Operation],[MatchAgainst],[MatchLimit],[RerollLimit]}";
    output["Arguments"] = {
      Operation: [
        "Use 'lt' for '<'",
        "Use 'gt' for '>'",
    ...
    output["Description"] =
      "Rerolls dice using the given Operation against MatchAgainst for MatchLimit dice up to a maximum of RerollLimit times each.";
    output["Examples"] = {
      "{r,e,1}": "Reroll any 1's as many times as needed.",
      "{r,gt,4,2}":
        "Reroll the first 2 dice greater than 4 as many times as needed.",
      "{r,lte,2,3,4}":
        "Reroll the first 3 dice less than or equal to 2 only four times each and then stop.",
      "{r,ne,2,,2}":
        "Reroll any dice not equal to 2 only twice each and then stop.",
    };
    return output;
  }
  ...
  rethrowAdvancedHelp() {
    ...
    output["Name"] = "Reroll";
    output["Usage"] =
      "{ra,[Operation],[MatchAgainst],[MatchLimit],[RerollLimit]}";
    output["Arguments"] = {
      Operation: [
        "Use 'lt' for '<'",
        "Use 'gt' for '>'",
  ...
  filterHelp() {
    ...
    output["Description"] =
      "Ignores any dice results using the given Operation against MatchAgainst for MatchLimit dice.";
    output["Examples"] = {
  ...
  advantage(dicemeshList, args) {
    ...
    for (let i = 0, len = dicemeshList.length; i < len; ++i) {
      ...
      highest =
        highest.getLastValue().value >= dicemesh.getLastValue().value
          ? highest
          : dicemesh;
    }
  ...
  disadvantage(dicemeshList, args) {
    ...
    for (let i = 0, len = dicemeshList.length; i < len; ++i) {
      ...
      lowest =
        lowest.getLastValue().value <= dicemesh.getLastValue().value
          ? lowest
          : dicemesh;
    }
