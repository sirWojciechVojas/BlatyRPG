...
export class DiceColors {
  ...
  applyColorSet(colorsetid, texture = "", material = "", update = true) {
    ...
    if (!diceRoller || !diceRoller.DiceFactory || !diceRoller.DiceFavorites) {
      return;
    }
    ...
    if (texture != "")
      diceRoller.DiceFactory.applyTexture(this.getTexture(texture));
    if (material != "") diceRoller.DiceFactory.applyMaterial(material);

    if (update) {
      // Sprawdzenie istnienia elementów przed próbą użycia
      let colorElement = Teal.id("color");
      let textureElement = Teal.id("texture");
      let materialElement = Teal.id("material");

      if (colorElement) {
        Teal.selectByValue(colorElement, colorsetid);
      }
      if (texture != "" && textureElement) {
        Teal.selectByValue(textureElement, texture);
      }
      if (material != "" && materialElement) {
        Teal.selectByValue(materialElement, material);
      }
    ...
    if (update && urlargs.length > 0) {
      let colorNameElement = Teal.id("colorname");
      if (colorNameElement) {
        Teal.empty(colorNameElement);
        colorNameElement.innerHTML =
          "Theme: " +
