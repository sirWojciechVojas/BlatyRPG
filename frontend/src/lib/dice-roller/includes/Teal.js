...
export class Teal {
  ...
  constructor() {
    ...
    // this.socketAddress = `ws://localhost:8080?campaign_id=${this.campaignInfo.id}`;
    this.socketAddress = `ws://localhost:8080?campaign_id=3`;
  ...
  static copyto(obj, res) {
    ...
    if (obj instanceof Array) {
      for (let i = obj.length - 1; i >= 0; --i) res[i] = Teal.copy(obj[i]);
    } else {
      for (let i in obj) {
  ...
  static id(id) {
    const myIdElement = document.getElementById(id);

    if (!myIdElement) {
      console.error("myIdElement +id is null or undefined:", id);
    }
    return myIdElement;
  }
  ...
  static empty(sel) {
    if (!sel || !sel.childNodes) {
      console.error("Element is null or does not have child nodes.");
      return;
    }
  ...
  openSocket(address, secure) {
    address =
      secure || this.socketSecure
        ? "wss://" + (address || this.socketAddress)
        : "ws://" + (address || this.socketAddress);
